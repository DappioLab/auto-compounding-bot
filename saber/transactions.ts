...
export async function createDepositTx(swapInfo: SwapInfo, AtokenAmount: BN, BtokenAmount: BN, minimalRecieve: BN, wallet: PublicKey, connection: Connection) {
  ...
  const AtokenSourceAccount = await findAssociatedTokenAddress(wallet, swapInfo.mintA);
  const createAtokenAccount = await createATAWithoutCheckIx(wallet, swapInfo.mintA);
  tx.add(createAtokenAccount);

  // check if Token B source account is created
  const BtokenSourceAccount = await findAssociatedTokenAddress(wallet, swapInfo.mintB);
  const createBtokenAccount = await createATAWithoutCheckIx(wallet, swapInfo.mintB);
  tx.add(createBtokenAccount);

  // check if LP Token account is created
  const LPtokenAccount = await findAssociatedTokenAddress(wallet, swapInfo.poolMint);

  // if false add a create IX
  const createLPtokenAccount = await await createATAWithoutCheckIx(wallet, swapInfo.poolMint);
  ...
  if (swapInfo.mintAWrapped) {
    ...
    const wrapMintAtokenAddress = await findAssociatedTokenAddress(wallet, swapInfo.mintAWrapInfo?.underlyingWrappedTokenMint as PublicKey);
    const createAtokenAccount = await createATAWithoutCheckIx(wallet, swapInfo.mintAWrapInfo?.underlyingWrappedTokenMint as PublicKey);
    tx.add(createAtokenAccount);

    const multiplyer = (new BN(swapInfo.mintAWrapInfo?.multiplyer as BN));
    const wrapAIns = ins.wrapToken(swapInfo.mintAWrapInfo as WrapInfo, wallet, AtokenAmount.div(multiplyer), wrapMintAtokenAddress, AtokenSourceAccount);
  ...
  if (swapInfo.mintBWrapped === true) {
    // check underlying tokan account is created
    const wrapMintBtokenAddress = await findAssociatedTokenAddress(wallet, swapInfo.mintBWrapInfo?.underlyingWrappedTokenMint as PublicKey);

    const createBtokenAccount = await createATAWithoutCheckIx(wallet, swapInfo.mintBWrapInfo?.underlyingWrappedTokenMint as PublicKey);
    tx.add(createBtokenAccount);

    const multiplyer = (new BN(swapInfo.mintBWrapInfo?.multiplyer as BN))
    const wrapBIns = ins.wrapToken(swapInfo.mintBWrapInfo as WrapInfo, wallet, BtokenAmount.div(multiplyer), wrapMintBtokenAddress, BtokenSourceAccount);

    tx.add(wrapBIns);
  }

  const depositIns = ins.deposit(swapInfo, AtokenAmount, BtokenAmount, minimalRecieve, wallet, AtokenSourceAccount, BtokenSourceAccount, LPtokenAccount)
  tx.add(depositIns);

  if (swapInfo.isFarming) {
    const farm = swapInfo.farmingInfo as FarmInfo;
    const depositToFarmIns = await depositToFarm(farm, wallet, minimalRecieve, connection);
...
export async function createMiner(farm: FarmInfo, wallet: PublicKey, connection: Connection) {
  ...
  const minerVault = await findAssociatedTokenAddress(miner[0], farm.tokenMintKey);
  if (!(await minerCreated(wallet, farm, connection))) {
    const createAtaIx = await createATAWithoutCheckIx(miner[0], farm.tokenMintKey,wallet);
    tx.add(createAtaIx)
    const createMinerIx = await ins.createMinerAccountIx(farm as FarmInfo, wallet);
    tx.add(createMinerIx);
  }
...
export async function createWithdrawTx(swapInfo: SwapInfo, tokenType: string, farmTokenAmount: BN, LPtokenAmount: BN, minimalRecieve: BN, wallet: PublicKey, connection: Connection) {
  const tx: Transaction = new Transaction();
  const cleanupTx = new Transaction();
  const LPtokenSourceAccount = await findAssociatedTokenAddress(wallet, swapInfo.poolMint);
  ...
  const createLPtokenAccount = await createATAWithoutCheckIx(wallet, swapInfo.poolMint);
  tx.add(createLPtokenAccount);
  const recieveTokenAccount = await findAssociatedTokenAddress(wallet, recieveTokenAccountMint);
  const createrecieveTokenAccount = await createATAWithoutCheckIx(wallet, recieveTokenAccountMint);
  tx.add(createrecieveTokenAccount);

  if (swapInfo.isFarming) {
    const farm = swapInfo.farmingInfo as FarmInfo;
    const withdrawFromfram = await withdrawFromMiner(farm, wallet, farmTokenAmount);
  ...
  if (tokenType === "A" && swapInfo.mintAWrapped) {
    const wrappedmint = swapInfo.mintAWrapInfo?.underlyingWrappedTokenMint as PublicKey;
    const mintAUnderlyingTokenAccount = await findAssociatedTokenAddress(wallet, wrappedmint);

    // if false add a create IX
    const createmMintAUnderlyingTokenIx = await await createATAWithoutCheckIx(wallet, wrappedmint);
    ...
    const wrappedmint = swapInfo.mintBWrapInfo?.underlyingWrappedTokenMint as PublicKey;
    const mintBUnderlyingTokenAccount = await findAssociatedTokenAddress(wallet, wrappedmint);
    const mintBUnderlyingTokenAccountCreated = await checkTokenAccount(mintBUnderlyingTokenAccount, connection)

    if (!mintBUnderlyingTokenAccountCreated) {
      // if false add a create IX
      const createMintBUnderlyingTokenAccount = await createATAWithoutCheckIx(wallet, wrappedmint);
...
export async function withdrawFromMiner(farm: FarmInfo, wallet: PublicKey, amount: BN): Promise<Transaction> {
  const tx = new Transaction()

  if (!amount.eq(new BN(0))) {
    tx.add(await createATAWithoutCheckIx(wallet, farm.tokenMintKey));

    const withdrawFromFarmIns = await ins.withdrawFromFarmIx(farm, wallet, amount);
...
export async function claimRewardTx(farm: FarmInfo, wallet: PublicKey, conn: Connection): Promise<Transaction> {
  const tx = new Transaction();
