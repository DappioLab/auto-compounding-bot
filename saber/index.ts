...
export async function getAllSwaps(conn: Connection): Promise<SwapInfo[]> {
  ...
  const allSaberAccount = await conn.getProgramAccounts(SWAP_PROGRAM_ID, config);
  ...
  for (const account of allSaberAccount) {
    ...
    const saberAccountInfo = await parseSwapInfoData(account.account.data, account.pubkey);
    ...
    const mintBwrapped = await checkWrapped(saberAccountInfo.mintB, wrapInfoArray)
...
export async function getSwap(connection: Connection, swap:PublicKey): Promise<SwapInfo> {
  const adminIdMemcmp: MemcmpFilter = {
    memcmp: {
      offset: 75,
      bytes: ADMIN_KEY.toString(),
