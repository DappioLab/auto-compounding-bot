...
export async function getAllTokenAccount(wallet: PublicKey, connection: Connection) {
  let tokenAccountInfos = await (await connection.getTokenAccountsByOwner(wallet, { programId: TOKEN_PROGRAM_ID })).value;
  let tokenAccounts = [];
  for (let info of tokenAccountInfos) {
    let tokenAccount = parseTokenAccount(info.account.data, info.pubkey);
...
export async function signAndSendAll(allTx: Transaction, connection: Connection, wallet: Keypair) {
  ...
  let result = sendAndConfirmTransaction(connection, tx, [wallet]);
  return result;
}