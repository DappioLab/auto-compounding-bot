...
import { Connection, PublicKey, MemcmpFilter, GetProgramAccountsConfig, DataSizeFilter } from "@solana/web3.js";
import { QURARRY_MINE_PROGRAM_ID, SABER_WRAP_PROGRAM_ID, SWAP_PROGRAM_ID} from './ids';
import { FARM_LAYOUT, MINER_LAYOUT, SWAPINFO_LAYOUT, WRAPINFO_LAYOUT } from "./layouts";
...
export async function getMinerKey(wallet: PublicKey, farmPubkey: PublicKey): Promise<[PublicKey, number]> {
...
export async function minerCreated(wallet: PublicKey, info: FarmInfo, connection: Connection): Promise<boolean> {
...
export class MinerInfo {
  ...
  getUnclaimedRewards(swapInfo: SwapInfo) {
    if (swapInfo.farmingInfo?.infoPubkey.toString() === this.farmKey.toString()) {
      const unClaim = this.balance.mul(
      swapInfo.farmingInfo.rewardsPerTokenStored
          .sub(this.rewardsPerTokenPaid)
      ).div(
      new BN([255, 255, 255, 255, 255, 255, 255, 255])
      ).add(
      this.rewardsEarned
...
export async function getAllMiners(connection: Connection, wallet: PublicKey): Promise<MinerInfo[]> {
  const adminIdMemcmp: MemcmpFilter = {
    memcmp: {
      offset: 8 + 32,
  ...
  for (const account of allMinerAccount) {
    const currentFarmInfo = parseMinerInfo(account.account.data, account.pubkey);
...
export async function parseSwapInfoData(data: any, pubkey: PublicKey): Promise<SwapInfo> {
  ...
  const authority = (await PublicKey.findProgramAddress([pubkey.toBuffer()], SWAP_PROGRAM_ID))[0];
  const { isInitialized,
    isPaused,
    nonce,
    initialAmpFactor,
  ...
  const swapInfo = new SwapInfo(pubkey,
    authority,
    isInitialized,
    isPaused,
    new BN (nonce),
...
export function parseWrapInfoData(data: any): WrapInfo {
  ...
  const { wrapAuthority, decimal, multiplyer, underlyingWrappedTokenMint, underlyingTokenAccount, wrappedTokenMint } = decodedData;
  const wrap = new WrapInfo(wrapAuthority, decimal, multiplyer, underlyingWrappedTokenMint, underlyingTokenAccount, wrappedTokenMint);
  return wrap;
}
